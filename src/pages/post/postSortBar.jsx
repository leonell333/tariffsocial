...
import {getPosts, updatePostStore} from '../../store/actions/postActions'
          ...
          <TrendingIcon/>
                ...
                <div className="flex items-center justify-end">
                    <Select value={sort} onValueChange={setSort}>
                        <SelectTrigger
                            className="border-none shadow-none p-0 h-auto w-auto bg-transparent focus:outline-none">
                            <SelectValue>
                                <span className="text-sm text-[#161722] font-semibold capitalize">{sort}</span>
                            </SelectValue>
                        </SelectTrigger>
                        <SelectContent className="bg-white border-none shadow-none outline-none disableScrollLock"
                                       side="bottom" align="start" alignOffset={-60} avoidCollisions={false}>
                            {['popular', 'recent', 'trending'].map((option) => (
                                <SelectItem key={option} value={option}
                                            className="hover:bg-gray-100 text-sm capitalize cursor-pointer">
                                    {option}
                                </SelectItem>
                            ))}
                        </SelectContent>
