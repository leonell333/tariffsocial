...
import {getBannerAds, getHashTags, getPostAndSponsored, getPosts, getSponsored, listenForNewPosts, searchPostsByKeywords, 
  updatePostStore} from "../../store/actions/postActions";
import CreatePost from "../../components/post/create";
import PostSortBar from "./postSortBar.jsx";
...
const Posts = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (isScrollingDown && isNearBottom && !isGeting) {
        setIsGeting(true);
        if (isSearchMode) {
          dispatch(searchPostsByKeywords(keyword, false))
            .then((res) => {
              setIsGeting(false);
            })
            .catch((err) => {
  ...
  const initData = async () => {
    try {
      ...
      if (promises.length > 0) {
        ...
        console.log("No initial data fetch needed");
      }
    } catch (err) {
  ...
  return (
    <div ref={postRef}>
      {user.authenticated && (
        <div className={`${showCreatePost ? `fixed z-40 top-[121px] bg-[#ECECEC] pb-1` : ""}`}>
          <div
          ...
          {isSearchMode ? (
            <>
              {searchPosts.length > 0 ? (
                searchPosts.map((item) => <Post key={item.id} post={item} />)
              ) : (
